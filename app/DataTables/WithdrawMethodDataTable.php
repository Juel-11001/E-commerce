    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query){
                ...
                $deleteBtn= "<a href='".route('admin.withdraw-method.destroy', $query->id)."'class='btn btn-danger ml-2 delete-item'><i class='fas fa-trash'></i></i></a>";
                return $editBtn.$deleteBtn;
            })
            ->addColumn('withdraw_charge', function($query){
                return $query->withdraw_charge.'%';
            })
            ->rawColumns(['action', 'withdraw_charge'])
            ->setRowId('id');
    }
