...
class BrandDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function($query){
                ...
                $deleteBtn= "<a href='".route('admin.brand.destroy', $query->id)."'class='btn btn-danger ml-2 delete-item'><i class='fas fa-trash'></i></i></a>";
                return $editBtn.$deleteBtn;
            })
            ->addColumn('logo', function($query){
                return $query->logo ? '<img src="'.asset($query->logo).'" alt="'.$query->name.'" width="100px">' : '';
            })
            ->addColumn('Is Featured', function($query){
            ...
            ->rawColumns(['logo', 'action', 'Is Featured', 'status'])
            ->setRowId('id');
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('name'),
            Column::make('Is Featured')->width(160),
